...
// Hold the UART logic in reset for a short time after configuration.
...
begin
	...
	case(state)
		...
		begin
			...
			else if(wait_cnt >= CLK_FRE * 1000000 && rx_data_valid==1'b0)// 
				state <= SEND;
		end
		default:
			state <= IDLE;
...
/*************************************************************************
combinational logic  Send "HELLO World\r\n"
****************************************************************************/
always@(*)
begin
	case(tx_cnt)
