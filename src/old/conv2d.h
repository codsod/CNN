...
public:
    ...
    static constexpr int IN_VEC = IN_D * BR_D; // 15*410 每通道
    static constexpr int OUT_VEC = BR_D;       // 410 每通道

    static SCALE_T get_branch_scale(int ic)
    {
        if (ic < BR0_CHANNELS)
        {
            return CONV_BR0_GELU_OUTPUT_SCALE;
        }
        if (ic < BR0_CHANNELS + BR1_CHANNELS)
        {
            return CONV_BR1_GELU_OUTPUT_SCALE;
        }
        return CONV_BR2_GELU_OUTPUT_SCALE;
    }

    static ZP_T get_branch_zp(int ic)
    {
        if (ic < BR0_CHANNELS)
        {
            return CONV_BR0_GELU_OUTPUT_ZP;
        }
        if (ic < BR0_CHANNELS + BR1_CHANNELS)
        {
            return CONV_BR1_GELU_OUTPUT_ZP;
        }
        return CONV_BR2_GELU_OUTPUT_ZP;
    }

    static if_t requant_to_conv2_domain(if_t x, int ic)
    {
        SCALE_T in_scale = get_branch_scale(ic);
        ZP_T in_zp = get_branch_zp(ic);
        float scaled =
            ((int)x - (int)in_zp) * ((float)in_scale / (float)CONV_SPATIAL_INPUT_SCALE) +
            (float)CONV_SPATIAL_INPUT_ZP;
        int v = round_to_even(scaled);
        return (if_t)clamp(v, 0, 255);
    }
...
#pragma HLS PIPELINE II = 1
                        buf[ic][row][t] = requant_to_conv2_domain(v[row * BR_D + t], ic);
                    }
                }
            }
...
#pragma HLS UNROLL
                    ...
                    float r = (float)CONV_SPATIAL_INPUT_SCALE * (float)CONV_SPATIAL_WEIGHT_SCALES /
